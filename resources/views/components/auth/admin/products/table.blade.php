          ...
          <x-auth.admin.products.data>
            <div>
              ...
              <x-auth.admin.products.modal logo="bi bi-trash" background="btn-danger" header="Deletion"
                target="delete_id_{{ $device->id }}">
                <div class="d-flex justify-content-between align-items-center">
                  <div class="fs-5">
                    <button class="fs-5 btn" data-bs-dismiss="modal">Cancel</button>
                  </div>
                  <form action="{{ route('admin.products.variants.delete', $device->id) }}"
                    id="confirm_id_{{ $device->id }}" method="POST">
                    @csrf
                    @method('DELETE')
                  </form>
                  <div class="fs-5 d-flex align-items-center">
                    <button class="fs-5 btn btn-danger" form="confirm_id_{{ $device->id }}">Confirm</button>
                  </div>
                </div>
